(() => {
  ...
  const newVideoLoaded = async () => {
    ...
    if (!bookmarkBtnExists) {
      ...
      ytLeftCtrls = document.getElementsByClassName("ytp-left-controls")[0];
      ytPlayer = document.getElementsByClassName("video-stream")[0];

      ytLeftCtrls.appendChild(bookmarkBtn);
  ...
  chrome.runtime.onMessage.addListener((obj, sender, response) => {
    ...
    if (type === "NEW") {
      // if a new video is loaded => if user watches a new (not the old video the user was watching..) video
