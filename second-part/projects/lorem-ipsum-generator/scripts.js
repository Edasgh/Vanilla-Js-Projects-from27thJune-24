...
//getting the form-field, the no. of paragraps input and the paragraphs container

const formField = document.getElementById("form-field");
const paragraphNoInput = document.getElementById("para-no");
...
const generateParagraphs = (value) => {
  ...
  if (num <= 0) {
    ...
    //if the given input is greater than the length of the array
    if (num > paragraphs.length) {
      // shuffle the array
      let arr = shuffle(paragraphs);
      let generatedResult = arr.slice(0, num);
      //calculating the difference between the given input and the length of the array
      let diff = num - paragraphs.length;
      for (let i = 0; i < diff; i++) {
        //then generate other paragraphs randomly from the array
        let randomIndx = Math.floor(Math.random() * diff);
        generatedResult.push(paragraphs[randomIndx]);
      }
